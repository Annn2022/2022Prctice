...
public class EventSetting : MonoBehaviour {
	...
	public void Activate(){
		...
		if(type == ParametersType.PlayLegacyAnimation){
			...
			if(t.GetComponent<PlayerData>() && t.GetComponent<PlayerData>().mainModel){
				t = t.GetComponent<PlayerData>().mainModel.transform;
			}
		...
		if(type == ParametersType.ChangeScene){
			...
			if(boolVar){
				mainPlayer.GetComponent<PlayerData>().spawnPointName = stringVar2;
			}
		...
		if(type == ParametersType.FreezeAll){
			GlobalCondition.freezeAll = boolVar;
			NextEvent();
		}
		...
		if(type == ParametersType.SetParent){
			if(boolVar && GlobalCondition.mainPlayer){
				target2 = GlobalCondition.mainPlayer.transform;
			}
			if(target && target2){
		...
		if(type == ParametersType.SetCameraTarget){
			...
			GlobalCondition.freezeCam = boolVar2;
		...
		if(type == ParametersType.LockCamera){
			GlobalCondition.freezeCam = boolVar;
			NextEvent();
		}
		
		if(type == ParametersType.AddCash){
			if(GlobalCondition.mainPlayer){
				GlobalCondition.mainPlayer.GetComponent<Inventory>().cash += intVar;
			}
			NextEvent();
		}
		if(type == ParametersType.AddSkillPoint){
			if(GlobalCondition.mainPlayer){
				GlobalCondition.mainPlayer.GetComponent<PlayerData>().skillPoint += intVar;
			}
			NextEvent();
		}
		if(type == ParametersType.AddStatusPoint){
			if(GlobalCondition.mainPlayer){
				GlobalCondition.mainPlayer.GetComponent<PlayerData>().statusPoint += intVar;
			}
			NextEvent();
		}
		if(type == ParametersType.AddItem){
			if(GlobalCondition.mainPlayer){
				GlobalCondition.mainPlayer.GetComponent<Inventory>().AddItem(intVar , intVar2);
			}
			NextEvent();
		}
		if(type == ParametersType.AddEquipment){
			if(GlobalCondition.mainPlayer){
				GlobalCondition.mainPlayer.GetComponent<Inventory>().AddEquipment(intVar);
			}
			NextEvent();
		}
		if(type == ParametersType.AddQuest){
			if(GlobalCondition.mainPlayer){
				//GlobalCondition.mainPlayer.GetComponent<QuestStatC>().AddQuest(intVar);
			}
			NextEvent();
		}
	}
	...
	Transform SetTarget(){
		if(targetIsPlayer){
			if(!GlobalCondition.mainPlayer){
				GlobalCondition.mainPlayer = GameObject.FindWithTag("Player");
			}
			if(!GlobalCondition.mainPlayer){
				print("No Player in the scene");
				return null;
			}
			return GlobalCondition.mainPlayer.transform;
		}else{
			if(!target){
				print("You didn't assign Target yet");
				return null;
