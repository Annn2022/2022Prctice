...
public class EventActivator : MonoBehaviour{
	...
	void FreezePlayer(){
		...
		onInteracting = true;
		if(lockCam){
			GlobalCondition.freezeCam = true;
		}
	...
	public void ActivateTrigger(){
		if(startCondition == StartConditions.KeyTrigger && !GlobalCondition.freezeAll && Time.timeScale != 0.0f && !GlobalCondition.freezePlayer){
			ActivateEvent();
		}
	}
	...
	public void EndEvent(){
		if(freezePlayerDuringEvent){
			...
			onInteracting = false;
			if(lockCam){
				GlobalCondition.freezeCam = false;
			}
