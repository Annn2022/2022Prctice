...
public class SkillTreeCanvasC : MonoBehaviour {
	...
	void Update(){
		...
		if(skillPointText){
			skillPointText.text = player.GetComponent<PlayerData>().skillPoint.ToString();
		}
	}
	...
	public void CheckUnlockSkill(){
		...
		CheckLearnedSkill();
		SkillWindowC sk = player.GetComponent<SkillWindowC>();
		int lv = player.GetComponent<PlayerData>().level;
	...
	public void LearnSkill(int buttonId){
		if(player.GetComponent<PlayerData>().skillPoint < skillSlots[buttonId].skPointUse){
			print("Not enough Skill Point");
			return;
		}
		player.GetComponent<PlayerData>().skillPoint -= skillSlots[buttonId].skPointUse;
		SkillWindowC sk = player.GetComponent<SkillWindowC>();
